...
class ServiceDiscovery extends ServiceBase {
  ...
  @backgroundMethod()
  fetchDiscoveryHomePageBannerData() {
    return this._fetchDiscoveryHomePageBannerData();
  }

  _fetchDiscoveryHomePageBannerData = memoizee(
    async () => {
      const res = await axios.get(`${CUSTOM_NETWORK_URL}/api/bus/Nav`);
      return res.data.data;
    },
    {
      promise: true,
  ...
  async fetchDAppList() {
    ...
    //   },
    // });
    const res = await axios.get(`${CUSTOM_NETWORK_URL}/api/bus/Browse`);
    return res.data.data;
  }

  @backgroundMethod()
  async fetchAnnouncements() {
    const res = await axios.get(`${CUSTOM_NETWORK_URL}/api/bus/Notice`);
    return res.data.data;
  }
