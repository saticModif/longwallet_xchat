...
class ContextJotaiActionsDiscovery extends ContextJotaiActionsBase {
  ...
  handleOpenWebSite = contextAtomMethod(
    (
      ...
      // 检查URL是否匹配指定域名
      const shouldInject = injectDomains.some(domain => url.includes(domain));
      console.log('shouldInject', shouldInject);
      // 准备注入的JS代码
      const injectedJS = shouldInject
        ? `
        (function() {
      
          window.oneKeyInjected = true;
          alert('injected');
        })();
      `
        : '';
