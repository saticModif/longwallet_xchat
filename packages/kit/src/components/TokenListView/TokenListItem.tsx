...
function BasicTokenListItem(props: ITokenListItemProps) {
  ...
  axios({
    method: 'get',
    url: 'https://api.zenex.one/market/symbol-thumb'
  }).then((res) => {
    const result = res.data;
    const chg = result.filter(result => result.symbol === 'ISPAY/USDT');
    // console.error(">>>>>>>>>>>",chg)
    if (chg?.length >= 1) {
      setPrice(chg[0].chg);
      setClose(chg[0].usdRate);
      AsyncStorage.setItem("ispayPrice", String(chg[0].usdRate))
      .then(()=>{
        // console.log("ddvdgfgefsa")
      }).catch(()=>{
        // console.error("dkkkkkkkk")
      })
    }
  })
    ...
    >
      ...
      >
        ...
        {isTokenSelector ? (
          ...
          >
            {/* {tableLayout ? ( */}
              <TokenPriceView symbol={token.symbol} close={close} $key={token.$key ?? ''} size="$bodyMd" />
            {/* ) : null} */}
            <TokenPriceChangeView price={price} symbol={token.symbol} $key={token.$key ?? ''} size="$bodyMd" />
          </XStack>
      ...
      <Stack
        flexDirection={isTokenSelector ? 'column' : 'column'}
        alignItems="flex-end"
        flexShrink={1}
        {...(tableLayout && {
          flexDirection: 'row',
      ...
      >
        ...
        {/* {withPrice ? (
          ...
          >
            ...
            <TokenPriceChangeView price={price} tokenVale={token} $key={token.$key ?? ''} size="$bodyMd" />
          </XStack>
        ) : null} */}
        {/* {isTokenSelector ? ( */}
          <TokenBalanceView
