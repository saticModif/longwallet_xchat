...
import { Icon, Page, Stack, Tab, YStack, Dialog } from '@onekeyhq/components';
...
import { useAutoAddNetwork, useInitLanguage } from './service';
let CONTENT_ITEM_WIDTH: Animated.Value | undefined;

const getDeviceDensity = () => {
  const { width, height } = Dimensions.get('window');
  const pixelDensity = PixelRatio.get();

  // 计算屏幕的物理尺寸
  const screenWidthInDp = width / pixelDensity;
  const screenHeightInDp = height / pixelDensity;
...
export function HomePageView({
  ...
  const deviceDensity = getDeviceDensity();
  console.log(`Device density: ${deviceDensity}`);
  useEffect(() => {
  ...
  const { addNetwork, } = useAutoAddNetwork();
