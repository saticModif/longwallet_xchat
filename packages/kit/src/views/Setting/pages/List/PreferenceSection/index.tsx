...
const ThemeListItem = () => {
  ...
  const [backgroundColor,setBackgroundColor] = useSettingsPersistAtom();
  const intl = useIntl();
...
const LanguageListItem = () => {
  ...
  const onChange = useCallback(async (text: string) => {
    ...
    let effectiveLocale = text;
    
    if (text === 'system') {
      effectiveLocale = Localization.locale;
    }

    const code = localeMap[effectiveLocale] ?? 1;
    // 仅存储编号
    setSettings((prev) => ({
      ...prev,
      localeCodeNumber: code,
    }));
